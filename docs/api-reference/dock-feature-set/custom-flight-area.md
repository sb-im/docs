...
上云 API 开放自定义飞行区功能。用户可以将敏感地点设定为禁飞区，并将这些信息同步给项目内的大疆机场。当无人机执行任务时，它会自动绕行禁飞区，从而确保作业的安全和规范性。本功能通过自定义飞行区文件圈定飞行器的飞行区域，**点击下载[自定义飞行区文件协议模板](/files/api-reference/custom-flight-area-template.json)**。
...
[自定义飞行区](/api-integration/api-reference/superdock-hangar/custom-flight-area)