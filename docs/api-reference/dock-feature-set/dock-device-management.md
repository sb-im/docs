...
*   [飞行器设备属性](/api-integration/api-reference/aircraft/drone-properties)
*   [机场设备属性](/api-integration/api-reference/superdock-hangar/properties)
*   [设备管理（MQTT）](/api-integration/api-reference/superdock-hangar/device)
    *   **设备拓扑更新**  
        网关设备感知到与子设备通信的连接与断开，会向云端上报子设备的上下线状态。协议中`type`与`sub_type`的值请参照[设备类型和机型列表](/api-integration/cloud-api/device-types)章节找到对照。
    *   **设备属性推送**  
        设备属性划分为定频数据（osd）与状态数据（state），osd 属性会以 2 HZ定频上报，state属性会在属性变化时上报。不同的设备属性我们提供了不同的处理策略，使用不同的topic上报。设备属性分别介绍在：[飞行器设备属性](/api-integration/api-reference/aircraft/drone-properties)、[机场设备属性](/api-integration/api-reference/superdock-hangar/properties)与[遥控器设备属性](/api-integration/api-reference/aircraft/drone-properties)章节。
    *   **设备属性设置**
